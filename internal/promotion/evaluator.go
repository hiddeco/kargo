...
func ExprEnvWithOutputs(outputs promotion.State) ExprEnvOption {
	return func(env map[string]any) {
		env["outputs"] = outputs
	}
}
...
func ExprEnvWithTaskOutputs(alias string, outputs promotion.State) ExprEnvOption {
	return func(env map[string]any) {
...
func (p *StepEvaluator) Config(ctx context.Context, promoCtx Context, step Step) (promotion.Config, error) {
	if step.Config == nil {
		return nil, nil
	}
