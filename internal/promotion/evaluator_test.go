...
func TestStepEvaluator_BuildExprEnv(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opts: []ExprEnvOption{
				ExprEnvWithOutputs(promotion.State{
					"output1": "value1",
		...
		},
		{
			...
			opts: []ExprEnvOption{
				ExprEnvWithTaskOutputs("task::step1", promotion.State{
		...
		},
		{
			...
			opts: []ExprEnvOption{
				ExprEnvWithTaskOutputs("regular-step", promotion.State{
...
func TestStepEvaluator_BuildStepContext(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			assertions: func(t *testing.T, stepCtx *promotion.StepContext, err error) {
				require.NoError(t, err)
				...
				assert.Equal(t, promotion.State{"key": "value"}, stepCtx.SharedState)
				...
				assert.Equal(t, promotion.Config{"key": "value"}, stepCtx.Config)
			},
		},
		{
